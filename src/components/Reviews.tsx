...
const Reviews: React.FC = () => {
  return (
    <section id="reviews" className="py-32 bg-slate-50 border-b border-slate-100 relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-6 relative z-10">
        ...
        >
          <h2 className="text-red-600 font-bold uppercase tracking-[0.4em] text-[10px] mb-6">সফলতার গল্প</h2>
          <h3 className="text-5xl md:text-6xl font-display font-bold text-slate-900 mb-8 tracking-tighter">
            উদ্যোক্তাদের <span className="italic font-light text-red-600 text-6xl">অনুপ্রেরণা।</span>
          </h3>
          ...
          {reviews.map((review, i) => (
            ...
            >
              <div className="flex justify-between items-start mb-8">
                <Quote className="text-red-50 group-hover:text-red-600 transition-colors duration-500" size={40} strokeWidth={1.5} />
                <div className="flex gap-1">
                  {[...Array(review.rating)].map((_, i) => (
                    <Star key={i} size={14} className="fill-red-600 text-red-600 opacity-20 group-hover:opacity-100 transition-opacity" />
                  ))}
                </div>
              </div>
